...
public abstract class Folder {
    ...
    public synchronized int getNewMessageCount() throws MessagingException {
        ...
        if (!isOpen()) {
            ...
            for (int i = 1; i <= total; i++) {
                try {
                    if (getMessage(i).isSet(Flag.RECENT)) {
                        newmsgs++;
                    }
    ...
    public synchronized int getUnreadMessageCount() throws MessagingException {
        ...
        if (!isOpen()) {
            ...
            for (int i = 1; i <= total; i++) {
                try {
                    if (!getMessage(i).isSet(Flag.SEEN)) {
                        unread++;
                    }
                } catch (MessageRemovedException e) {
                }
    ...
    public synchronized int getDeletedMessageCount() throws MessagingException {
        ...
        if (!isOpen()) {
            ...
            for (int i = 1; i <= total; i++) {
                try {
                    if (getMessage(i).isSet(Flag.DELETED)) {
                        deleted++;
                    }
                } catch (MessageRemovedException e) {
                }
    ...
    private void queueEvent(MailEvent event, Vector vector) {
        synchronized (this.qLock) {
            if (this.f24q == null) {
                this.f24q = new EventQueue();
            }
        }
        this.f24q.enqueue(event, (Vector) vector.clone());
    }
    ...
    private void terminateQueue() {
        synchronized (this.qLock) {
            if (this.f24q != null) {
                ...
                this.f24q.enqueue(new TerminatorEvent(), dummyListeners);
                this.f24q = null;
            }
        }
    }
