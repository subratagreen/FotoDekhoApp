...
class SearchSequence {
    ...
    static Argument generateSequence(SearchTerm term, String charset) throws SearchException, IOException {
        ...
        if (term instanceof OrTerm) {
            return m2or((OrTerm) term, charset);
        }
    ...
    /* renamed from: or */
    private static Argument m2or(OrTerm term, String charset) throws SearchException, IOException {
        SearchTerm[] terms = term.getTerms();
        if (terms.length > 2) {
    ...
    private static Argument recipient(RecipientType type, String address, String charset) throws SearchException, IOException {
        Argument result = new Argument();
        if (type == RecipientType.f26TO) {
            result.writeAtom("TO");
        } else if (type == RecipientType.f25CC) {
            result.writeAtom("CC");
