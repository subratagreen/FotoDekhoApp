...
final class MarkableInputStream extends InputStream {
    ...
    /* renamed from: in */
    private final InputStream f3in;
    private long limit;
    private long offset;
    private long reset;
    ...
    private void setLimit(long limit2) {
        try {
            if (this.reset >= this.offset || this.offset > this.limit) {
                ...
                this.f3in.mark((int) (limit2 - this.offset));
            } else {
                this.f3in.reset();
                this.f3in.mark((int) (limit2 - this.reset));
    ...
    private void skip(long current, long target) throws IOException {
        while (current < target) {
            long skipped = this.f3in.skip(target - current);
            if (skipped == 0) {
                if (read() != -1) {
                    skipped = 1;
                } else {
    ...
    public int read(byte[] buffer) throws IOException {
        int count = this.f3in.read(buffer);
        if (count != -1) {
    ...
    public int read(byte[] buffer, int offset2, int length) throws IOException {
        int count = this.f3in.read(buffer, offset2, length);
        if (count != -1) {
            this.offset += (long) count;
        }
        return count;
    ...
    public long skip(long byteCount) throws IOException {
        long skipped = this.f3in.skip(byteCount);
        this.offset += skipped;
        return skipped;
    }

    public int available() throws IOException {
        return this.f3in.available();
    }
    ...
    public boolean markSupported() {
        return this.f3in.markSupported();
    }
}
